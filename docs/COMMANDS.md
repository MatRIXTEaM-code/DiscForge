...
DiscForge exposes **357 commands** (341 plus `dic-log` and `cold-case`, added 2026-09-18, `xdelta-apply`/`xdelta-info`/`xdelta-create`, added 2026-09-24, and `ace-list`/`ace-test`/`ace-extract` and `unpack`/`unpack-list`/`unpack-test`/`unpack-sweep`/`unpack-image`, added 2026-09-25, `rescue`/`rescue-status` and `protect`, added 2026-09-26 — this
...
rather than silently "fixed" by guessing which number is actually right).
Many read-only commands accept `--json` for machine-readable output.
...
- `rescue <drive|file> <out.iso> [map] [--retries N] [--no-trim] [--no-scrape] [--no-skip] [--cluster N] [--timeout S] [--start LBA] [--count N] [--no-slowdown] [--no-slow-skip] [--no-c2] [--c2-reads N] [--salvage]  Copy a damaged data disc GNU-ddrescue-style (plus optical extras: the drive is slowed down in damaged areas, single-sector reads use Force Unit Access and retries defeat the read cache, CD sectors are rebuilt from C2-guided raw reads + EDC/ECC, very slow areas are left for later; --salvage fills never-read sectors with the drive's uncorrected data but keeps them listed as bad): readable areas first in big reads (skipping away from errors, then a backwards pass and a sweep), then trim each failed area from its edges, scrape what's left sector by sector, and retry bad sectors N times. Progress is kept in a ddrescue-format map (default <out.iso>.map): Ctrl+C and re-run to continue, or re-run with another drive and the same image+map to read only what's missing. Writes <out.iso>.badsectors.json for any sectors still missing. Exit 0 = complete, 2 = sectors missing. Unencrypted discs only.`
- `rescue-status <map> [--json]  Summarise a rescue map (DiscForge's or GNU ddrescue's): size, rescued, non-tried / non-trimmed / non-scraped, bad sectors and bad areas.`
- `protect <create|verify|repair> <image> [parity.dfpar] [--level low|normal|high]  Reed-Solomon parity file for an image (≈3/7/14%): create it while the image is good; verify checks every image and parity sector against its stored CRC-32; repair rebuilds damaged sectors in place (any P lost sectors per interleaved group — a scratch-sized run is spread across groups).`
...
- `cold-case <registry.json> add/due/attempt/list  Tracks an INCOMPLETE dump for a future re-attempt instead of letting it become a forgotten one-shot failure — cleaning the disc, a different drive, or just a different day can change a read's outcome. add flags a dump with a reason and optional not-before date; due lists everything whose retry date has passed; attempt records a retry (--resolved closes the case, --retry-in-days N pushes the next one out); list shows every tracked case. Tracks WHETHER and WHEN to retry, nothing about HOW.`
...
- `dic-log <dump.log> [--json] [--to-bad-sectors out.badsectors.json --total-sectors N]  Imports a DiscImageCreator (DIC) .log file — DIC's log format is close to a de facto submission standard in the Redump community, and this reads one without needing to re-dump the disc through DiscForge. Extracts DIC version, drive identity, media type, per-track CRC32/MD5/SHA1, and C2 error LBAs; every field is optional since DIC's format was never formally specified and has drifted across versions. --to-bad-sectors converts the C2 error LBAs into a DiscForge bad-sector-map sidecar usable directly by redump-diff/dump-audit. redumper .log files are detected automatically and reported the same way (version, drive, write offset, SCSI/C2/Q error counts, and the dat block's per-file hashes); --to-bad-sectors isn't available for them.`
