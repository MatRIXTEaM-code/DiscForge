...
- `dic-log <dump.log> [--json] [--to-bad-sectors out.badsectors.json --total-sectors N]  Imports a DiscImageCreator (DIC) .log file — DIC's log format is close to a de facto submission standard in the Redump community, and this reads one without needing to re-dump the disc through DiscForge. Extracts DIC version, drive identity, media type, per-track CRC32/MD5/SHA1, and C2 error LBAs; every field is optional since DIC's format was never formally specified and has drifted across versions. --to-bad-sectors converts the C2 error LBAs into a DiscForge bad-sector-map sidecar usable directly by redump-diff/dump-audit.`
