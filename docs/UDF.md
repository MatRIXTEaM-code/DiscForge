...
Scope of the writer, honestly: UDF **1.02, 1.50, 2.00, 2.01 and 2.50** (`--udf-version`).
1.02/1.50 are structurally identical for a read-only random-access image, differing
only in the revision recorded in the OSTA domain identifiers and the integrity
descriptor. 2.00/2.01 additionally use **ECMA-167 3rd-edition descriptor tags
(version 3)** and an **Extended File Entry for every node** (the writer's extended-entry
path, previously used only for named streams, now covers all nodes when the target
revision is ≥ 2.00; the descriptor version is set once per build). **UDF 2.50** (Blu-ray)
additionally wraps the whole content in a **Metadata Partition** — a Type-2 partition map
plus a Metadata File and Mirror File Entry (file types 250/251) whose extent maps the
metadata-partition blocks to physical sectors. DiscForge keeps the content layout at
partition reference 0 (unchanged from ≤2.01) and adds the physical map at reference 1, so
the addressing stays simple. All five revisions are validated against `udfinfo`
(`udfrev=1.02/1.50/2.00/2.01/2.50`, no warnings) **and** round-trip through DiscForge's own
reader (which resolves the metadata partition, the same path a Blu-ray uses). Only **UDF
2.60** (pseudo-overwrite for BD-R) is still unsupported. Short_ad extents, ASCII/Latin-1
names, 2048-byte blocks.
**Streamed writing is done:** `UdfBuilder.BuildToStream` writes to a
...
- **Extended File Entries (tag 266)** — now exercised on the write→read path: authoring a node
  with a **named stream** emits a tag-266 Extended File Entry with a stream directory, and the
  reader resolves it back. `UdfStreamsEaTests` round-trips extended attributes (ECMA-167 4/14.10)
  and named streams (4/14.17) — identifiers, names, sizes and bytes — including multiple streams,
  a multi-sector stream, directory-level streams, and deterministic rebuilds. (An *external*
  genisoimage fixture still uses plain File Entries; the DiscForge round-trip is what covers 266.)
