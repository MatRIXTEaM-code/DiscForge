...
# 2) The CLI. It multi-targets net8.0 (cross-platform) and net8.0-windows (adds the
#    IMAPI2 optical-burn stack). The Windows installer ships the net8.0-windows build
#    so `dforge burn`/`drives` work; the framework MUST be named explicitly because a
#    multi-targeted project cannot be published without -f. It shares Core with the app,
#    so publishing into the same folder just adds dforge.exe (+ its deps json).
Write-Host "`nPublishing dforge (CLI)..." -ForegroundColor Yellow
dotnet publish "$repo\src\DiscForge.Cli\DiscForge.Cli.csproj" -f net8.0-windows @common -o $OutDir
