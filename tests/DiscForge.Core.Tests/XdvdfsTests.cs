...
public class XdvdfsTests
{
    ...
    [Fact]
    public void A_subdirectory_table_spanning_many_sectors_roundtrips()
    {
        // The existing multi-sector tests are all at the ROOT. This puts the big table one level
        // down, so the reader must recurse into a subdirectory AND resolve a table that spans
        // sectors — the two paths together, which nothing else covers.
        const int count = 300;
        var kids = new List<XdvdfsBuilder.Node>();
        for (int i = 0; i < count; i++)
            kids.Add(XdvdfsBuilder.Node.File($"asset{i:D4}.bin", Bytes($"payload-{i}")));

        var (vol, image) = BuildAndRead(
            XdvdfsBuilder.Node.File("default.xbe", Bytes("boot")),
            XdvdfsBuilder.Node.Dir("assets", kids.ToArray()));

        var subdir = vol.Directories.Single(d => d.Path == "/assets");
        Assert.True(subdir.Size % XdvdfsBuilder.SectorSize == 0);
        Assert.True(subdir.Size > XdvdfsBuilder.SectorSize);   // the SUBDIR table is genuinely multi-sector

        Assert.Equal(count, vol.Files.Count(f => f.Path.StartsWith("/assets/")));
        for (int i = 0; i < count; i++)
            Assert.Equal($"payload-{i}", Encoding.ASCII.GetString(Extract(image, vol, $"/assets/asset{i:D4}.bin")));
    }

    [Fact]
    public void Both_root_and_a_subdirectory_can_span_sectors_at_once()
    {
        // Root holds 200 files AND a subdirectory that itself holds 200 — two independent
        // multi-sector tables in one image, proving they don't interfere.
        var rootKids = new List<XdvdfsBuilder.Node>();
        for (int i = 0; i < 200; i++) rootKids.Add(XdvdfsBuilder.Node.File($"root{i:D3}.dat", Bytes($"r{i}")));
        var subKids = new List<XdvdfsBuilder.Node>();
        for (int i = 0; i < 200; i++) subKids.Add(XdvdfsBuilder.Node.File($"sub{i:D3}.dat", Bytes($"s{i}")));
        rootKids.Add(XdvdfsBuilder.Node.Dir("deep", subKids.ToArray()));

        var (vol, image) = BuildAndRead(rootKids.ToArray());

        Assert.True(vol.RootSize > XdvdfsBuilder.SectorSize);
        Assert.Equal(200, vol.Files.Count(f => !f.Path.StartsWith("/deep/")));
        Assert.Equal(200, vol.Files.Count(f => f.Path.StartsWith("/deep/")));
        // Spot-check a file from each big table.
        Assert.Equal("r0",   Encoding.ASCII.GetString(Extract(image, vol, "/root000.dat")));
        Assert.Equal("s199", Encoding.ASCII.GetString(Extract(image, vol, "/deep/sub199.dat")));
    }
    ...
    [Fact]
    public void A_volume_at_an_undocumented_base_is_found_by_the_scan()
    {
        // A dump whose partition begins at an offset that is NOT one of the four documented bases —
        // auto-detect must fall back to scanning for the descriptor signature and still find it.
        long baseSector = 1234;   // not 0 / 0x30600 / 0x1FB20 / 0x4100
        var xiso = XdvdfsBuilder.Build(new[] { XdvdfsBuilder.Node.File("default.xbe", Bytes("odd")) });
        var full = new byte[baseSector * 2048 + xiso.Length];
        Array.Copy(xiso, 0, full, baseSector * 2048, xiso.Length);

        using var ms = new MemoryStream(full);
        Assert.True(XdvdfsReader.IsXdvdfs(ms));
        var vol = XdvdfsReader.Read(ms);            // no explicit base
        Assert.Equal(baseSector, vol.BaseSector);
        Assert.Single(vol.Files, f => f.Path == "/default.xbe");
    }
