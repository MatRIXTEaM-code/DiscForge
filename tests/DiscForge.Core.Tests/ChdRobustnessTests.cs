...
/// <summary>
/// Robustness of CHD decoding against corrupt or hostile input. A malformed CHD must
/// be declined with a <see cref="ChdFormatException"/> — never crash with an
/// out-of-range/overflow exception, exhaust memory from a bogus size field, or hang in
/// a codec loop. (A fuzz sweep of thousands of mutations confirms this; these lock in
/// the specific guards.)
/// </summary>
public class ChdRobustnessTests
{
    // A small real hard-disk CHD (7 LZMA hunks + 1 NONE), used as a base to corrupt.
    ...
    private static byte[] Good() => System.Convert.FromBase64String(Chd);

    private static Exception? Caught(Action a)
    {
        try { a(); return null; } catch (Exception e) { return e; }
    }

    [Fact]
    public void A_truncated_chd_is_declined_not_crashed()
    {
        var good = Good();
        for (int len = 20; len < good.Length; len += 137)
        {
            var t = good[..len];
            var ex = Caught(() => ChdHdExtractor.Extract(t));
            Assert.True(ex is null or ChdFormatException, $"truncation to {len} threw {ex?.GetType().Name}");
        }
    }

    [Fact]
    public void An_implausibly_large_logical_size_is_refused_without_allocating()
    {
        var m = Good();
        BinaryPrimitives.WriteUInt64BigEndian(m.AsSpan(0x20), 0x7FFF_FFFF_FFFFul);  // ~140 TB
        var ex = Assert.Throws<ChdFormatException>(() => ChdHdExtractor.Extract(m));
        Assert.Contains("too large", ex.Message);
    }

    [Fact]
    public void A_zero_hunk_size_is_refused()
    {
        var m = Good();
        BinaryPrimitives.WriteUInt32BigEndian(m.AsSpan(0x38), 0);   // hunkbytes = 0
        Assert.Throws<ChdFormatException>(() => ChdHdExtractor.Extract(m));
    }

    [Fact]
    public void An_implausible_hunk_count_is_refused_by_the_map_decoder()
    {
        // 100 million hunks would need a 1.2 GB rawmap — refused, not honoured.
        var ex = Assert.Throws<ChdFormatException>(
            () => ChdMap.Decode(Good(), mapOffset: 0x100, hunkCount: 100_000_000, hunkBytes: 4096, unitBytes: 512));
        Assert.Contains("implausibly large", ex.Message);
    }

    [Fact]
    public void Corrupting_the_hunk_data_declines_cleanly()
    {
        // Flip bytes in the hunk-data region (after the header) many times; every
        // result must decline rather than hang or throw a non-domain exception.
        var rng = new System.Random(2026);
        var good = Good();
        for (int trial = 0; trial < 200; trial++)
        {
            var m = (byte[])good.Clone();
            for (int k = 0; k < 6; k++) m[200 + rng.Next(m.Length - 200)] = (byte)rng.Next(256);
            var ex = Caught(() => ChdHdExtractor.Extract(m));
            Assert.True(ex is null or ChdFormatException, $"corruption threw {ex?.GetType().Name}: {ex?.Message}");
        }
    }
}
