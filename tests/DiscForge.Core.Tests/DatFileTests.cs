...
public class DatFileTests
{
    ...
    [Fact]
    public void Md5_alone_verifies_with_md5_strength()
    {
        var dat = DatFile.ParseText(Dat);
        var m = dat.Verify(1000, crc: null, sha1: null, md5: "11112222333344445555666677778888");
        Assert.True(m.Verified);
        Assert.Equal("md5", m.Strength);
        Assert.Contains("Track 1", m.Rom!.Name);
    }

    [Fact]
    public void Strength_reflects_the_hash_that_proved_the_match()
    {
        var dat = DatFile.ParseText(Dat);
        Assert.Equal("sha1", dat.Verify(1000, "aabbccdd", "0123456789abcdef0123456789abcdef01234567").Strength);
        Assert.Equal("crc32", dat.Verify(88, "12345678").Strength);
        Assert.Null(dat.Verify(999, "00000000").Strength);
    }
}
