...
public class IpBinTests
{
    ...
    // ---- device-info CRC & integrity ---------------------------------------

    // Build a header whose device info carries the correct Katana boot CRC and a disc position.
    private static byte[] BuildWithCrc(string product = "MK-51035", string version = "V1.007",
                                       int discNo = 1, int discTotal = 1)
    {
        var m = BuildMeta(product: product, version: version);
        int crc = IpBin.BootCrc(m.AsSpan(0x40, 16));
        PutAscii(m, 0x20, 16, $"{crc:X4} GD-ROM{discNo}/{discTotal}");
        return m;
    }

    [Fact]
    public void Validates_the_device_info_checksum_against_the_product_fields()
    {
        var h = IpBin.Parse(BuildWithCrc());
        Assert.True(h.CrcPresent);
        Assert.True(h.CrcValid);
        Assert.Equal(h.ComputedCrc, h.StoredCrc);
        Assert.StartsWith("boot header intact", h.Integrity());
    }

    [Fact]
    public void A_tampered_product_field_breaks_the_checksum()
    {
        var m = BuildWithCrc();
        m[0x41] = (byte)'X';                 // mutate the product number after the CRC was written
        var h = IpBin.Parse(m);
        Assert.False(h.CrcValid);
        Assert.Contains("mismatch", h.Integrity());
    }

    [Fact]
    public void Parses_the_disc_number_and_total_from_a_multi_disc_header()
    {
        var h = IpBin.Parse(BuildWithCrc(discNo: 2, discTotal: 3));
        Assert.Equal(2, h.DiscNumber);
        Assert.Equal(3, h.DiscTotal);
    }

    [Fact]
    public void A_header_with_no_stored_checksum_reads_as_homebrew_not_corrupt()
    {
        // BuildMeta's default device info is "GD-ROM1/1" — no leading CRC token.
        var h = IpBin.Parse(BuildMeta());
        Assert.False(h.CrcPresent);
        Assert.False(h.CrcValid);
        Assert.Contains("homebrew or rebuilt", h.Integrity());
    }
}
