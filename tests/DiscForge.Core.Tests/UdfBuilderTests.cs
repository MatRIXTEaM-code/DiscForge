...
public class UdfBuilderTests
{
    ...
    /// <summary>
    /// Descriptors recorded inside a partition must carry a PARTITION-RELATIVE tag
    /// location (ECMA-167 4/7.2.1), so the File Set Descriptor at partition block 0
    /// must have tag location 0 — not the absolute sector it happens to sit on.
    /// DiscForge's own reader locates the FSD by position and so never noticed when
    /// this field held the absolute sector; strict readers (udfinfo, some OS drivers)
    /// followed the volume descriptor's pointer, found a tag whose location didn't
    /// match, and reported the File Set Descriptor as missing. This guards that fix
    /// straight from the image bytes, independent of the reader.
    /// </summary>
    [Fact]
    public void The_file_set_descriptor_has_a_partition_relative_tag_location()
    {
        var image = UdfBuilder.Build("DISC", new[] { UdfBuilder.Node.File("A.TXT", Bytes("hi")) });
        const int SS = 2048;

        // Anchor at sector 256 -> Main Volume Descriptor Sequence extent.
        int mvdsLen = BitConverter.ToInt32(image, 256 * SS + 16);
        int mvdsLoc = BitConverter.ToInt32(image, 256 * SS + 20);

        // Scan the MVDS for the Partition Descriptor (tag 5) to find where the partition starts.
        int partitionStart = -1;
        for (int s = mvdsLoc; s < mvdsLoc + mvdsLen / SS; s++)
            if (BitConverter.ToUInt16(image, s * SS) == 5)
                partitionStart = BitConverter.ToInt32(image, s * SS + 188);
        Assert.True(partitionStart > 0, "no Partition Descriptor found");

        // The File Set Descriptor sits at partition block 0 = absolute sector partitionStart.
        int fsd = partitionStart * SS;
        Assert.Equal(256, BitConverter.ToUInt16(image, fsd));            // tag id: File Set Descriptor
        Assert.Equal(0u, BitConverter.ToUInt32(image, fsd + 12));        // tag location: partition-relative 0
    }
