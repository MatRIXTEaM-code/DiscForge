...
public static class UdfBuilder
{
    ...
    /// <summary>Build a UDF 1.02 image from a tree of files and directories.</summary>
    ...
    public static BuildResult BuildResultOf(string volumeId, IEnumerable<Node> rootChildren)
    {
        ...
        uint imageSectors = PartitionStart + plan.PartitionBlocks + 1;
        ...
        WriteMainVds(image, MainVdsSector, volumeId, plan.PartitionBlocks);
        WriteMainVds(image, ReserveVdsSector, volumeId, plan.PartitionBlocks);
        WriteIntegrity(image, plan.PartitionBlocks, CountFiles(root), CountDirs(root));
        WriteAnchor(image, AnchorSector);
        WriteAnchor(image, imageSectors - 1);
        WriteFileSetDescriptor(image, root.FeBlock, volumeId);
    ...
    public static IReadOnlyList<string> BuildToStream(string volumeId, Stream output, IEnumerable<Node> rootChildren)
    {
        ...
        WriteMainVds(meta, MainVdsSector, volumeId, plan.PartitionBlocks);
        WriteMainVds(meta, ReserveVdsSector, volumeId, plan.PartitionBlocks);
        WriteIntegrity(meta, plan.PartitionBlocks, CountFiles(root), CountDirs(root));
        WriteAnchor(meta, AnchorSector);
        WriteFileSetDescriptor(meta, root.FeBlock, volumeId);
    ...
    private static void WriteMainVds(byte[] image, uint startSector, string volumeId, uint partitionBlocks)
    {
        ...
        WriteImplUseVolume(image, startSector + 1, volumeId);
        WritePartition(image, startSector + 2, partitionBlocks);
        WriteLogicalVolume(image, startSector + 3, volumeId);
    ...
    private static void WriteImplUseVolume(byte[] image, uint sector, string volumeId)
    {
        int at = (int)(sector * SectorSize);
        var s = image.AsSpan(at, 512);
        BinaryPrimitives.WriteUInt32LittleEndian(s[16..], 0);          // VDS number
    ...
    private static void WriteLogicalVolume(byte[] image, uint sector, string volumeId)
    {
        int at = (int)(sector * SectorSize);
        var s = image.AsSpan(at, 512);
        BinaryPrimitives.WriteUInt32LittleEndian(s[16..], 0);          // VDS number
        ...
        // Partition map 1 (Type 1, physical): type, length, vol seq, partition num.
        var map = s.Slice(440, 6);
        map[0] = 1;
        map[1] = 6;
    ...
    private static void WriteIntegrity(byte[] image, uint partitionBlocks, int files, int dirs)
    {
    ...
    private static void WriteFileSetDescriptor(byte[] image, uint rootFeBlock, string volumeId)
    {
        ...
        FinishTag(image, at, 512, TagFileSet, PartitionStart);         // tag location: partition block 0
    }
    ...
    private static void WriteRegid(Span<byte> s, string id, bool udfSuffix = false)
    {
        ...
        if (udfSuffix)
        {
            // UDF identifier suffix: UDF revision (1.02) then OS class/identifier.
            s[24] = 0x02;
            s[25] = 0x01;
        }
    }
