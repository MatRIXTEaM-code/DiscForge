...
public static class IsoBuilder
{
    ...
    public sealed class IsoLayout
    {
        ...
        internal int BootCatSector, BootImgSector, EfiBootImgSector;
    ...
    ///
    /// <paramref name="firstDataSector"/> relocates where the path tables,
    /// directory records and file payloads begin (the volume descriptors stay at
    /// 16/17/18). It defaults to -1, meaning "immediately after the descriptors"
    /// — the ordinary layout. The UDF-bridge builder passes a higher sector so the
    /// ISO 9660 content lands inside the shared UDF partition, past the UDF File
    /// Entries and File Identifier Descriptors, while the file *extents* it assigns
    /// remain the sectors both filesystems point at.
    /// </summary>
    public static IsoLayout Plan(string volumeId, IReadOnlyList<Node> rootChildren,
        bool joliet = true, BootImage? boot = null, bool rockRidge = false, int firstDataSector = -1,
        BootImage? efiBoot = null)
    {
        ...
        int pvdSector = 16;
        int nextVd = 17;
        bool hasBoot = boot is not null || efiBoot is not null;
        int bootRecSector = hasBoot ? nextVd++ : -1;
        int svdSector = joliet ? nextVd++ : -1;
        int termSector = nextVd++;
        // The bridge builder relocates the ISO content into the shared partition;
        // an override that is not past the descriptors would corrupt them, so clamp.
        int dataStart = firstDataSector >= nextVd ? firstDataSector : nextVd;
        ...
        int bootCatSector = -1, bootImgSector = -1, efiBootImgSector = -1;
        if (hasBoot)
        {
            bootCatSector = cursor; cursor += 1;
            if (boot is not null) { bootImgSector = cursor; cursor += CeilSectors(boot.Data.Length); }
            if (efiBoot is not null) { efiBootImgSector = cursor; cursor += CeilSectors(efiBoot.Data.Length); }
        }
        ...
        return new IsoLayout
        {
            ...
            EfiBootImgSector = efiBootImgSector,
    ...
    private static void WriteLayout(IsoLayout L, Stream output)
    {
        ...
        if (L.Boot is not null || L.EfiBoot is not null)
        ...
        if (L.Boot is not null || L.EfiBoot is not null)
        {
            WriteBootCatalog(meta.AsSpan(L.BootCatSector * SectorSize, SectorSize),
                             L.Boot, L.BootImgSector, L.EfiBoot, L.EfiBootImgSector);
            L.Boot?.Data.CopyTo(meta.AsSpan(L.BootImgSector * SectorSize));
            L.EfiBoot?.Data.CopyTo(meta.AsSpan(L.EfiBootImgSector * SectorSize));
        }
    ...
    private const byte ElToritoPlatformX86 = 0x00;
    private const byte ElToritoPlatformEfi = 0xEF;

    /// <summary>Write one 32-byte El Torito boot entry (default or section) at <paramref name="off"/>.</summary>
    private static void WriteBootEntry(Span<byte> cat, int off, BootImage img, int imageSector)
    {
        cat[off + 0] = 0x88;                    // boot indicator: bootable
        cat[off + 1] = (byte)img.Media;         // media type
        BinaryPrimitives.WriteUInt16LittleEndian(cat.Slice(off + 2, 2), 0);   // load segment: default
        cat[off + 4] = 0;                        // system type
        int sectorCount = Math.Max(1, (img.Data.Length + 511) / 512);         // virtual 512-byte sectors
        BinaryPrimitives.WriteUInt16LittleEndian(cat.Slice(off + 6, 2), (ushort)sectorCount);
        BinaryPrimitives.WriteUInt32LittleEndian(cat.Slice(off + 8, 4), (uint)imageSector);
    }

    /// <summary>
    /// Write the El Torito boot catalog. A BIOS (x86) image alone is the classic single-entry
    /// catalog; a UEFI image alone becomes an EFI-platform initial entry; both together produce a
    /// BIOS+UEFI HYBRID — an x86 initial entry plus a final section header (platform 0xEF) carrying
    /// the EFI entry — so one disc boots on legacy BIOS and UEFI firmware alike.
    /// </summary>
    private static void WriteBootCatalog(Span<byte> cat, BootImage? boot, int bootImageSector,
                                         BootImage? efiBoot, int efiImageSector)
    {
        bool biosInitial = boot is not null;    // BIOS present → x86 initial entry; else UEFI-only

        // Validation entry (bytes 0..31). Its platform matches the INITIAL entry.
        cat[0] = 1;                              // header id
        cat[1] = biosInitial ? ElToritoPlatformX86 : ElToritoPlatformEfi;
        cat[30] = 0x55;
        cat[31] = 0xAA;
        ushort sum = 0;
        ...
        // Default/initial entry (bytes 32..63): the BIOS image, or the EFI image on a UEFI-only disc.
        var initial = boot ?? efiBoot!;
        int initialSector = biosInitial ? bootImageSector : efiImageSector;
        WriteBootEntry(cat, 32, initial, initialSector);

        // Hybrid: append a final EFI section (header at 64, one section entry at 96).
        if (boot is not null && efiBoot is not null)
        {
            cat[64] = 0x91;                      // section header: final
            cat[65] = ElToritoPlatformEfi;       // platform: EFI
            BinaryPrimitives.WriteUInt16LittleEndian(cat.Slice(66, 2), 1);   // one entry follows
            WriteBootEntry(cat, 96, efiBoot, efiImageSector);
        }
    }
