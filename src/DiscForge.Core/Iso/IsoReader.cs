...
public sealed record IsoEntry
{
    ...
    /// <summary>The POSIX metadata from Rock Ridge (permissions, ownership, symlink target, timestamps),
    /// or null when the disc has no Rock Ridge extensions on this entry.</summary>
    public RockRidgeInfo? RockRidge { get; init; }
...
public static class IsoReader
{
                                                ...
                                                bool joliet, bool rockRidge, string prefix)
    {
        ...
        while (p < data.Length)
        {
            ...
            RockRidgeInfo? rr = null;
            if (rockRidge)
            {
                rr = ParseRockRidge(iso, rec, idLen);
                if (!string.IsNullOrEmpty(rr?.Name)) name = rr!.Name!;
            }
    ...
    /// <summary>Decode the full Rock Ridge / SUSP System Use area of one directory record,
    /// following CE continuation blocks into other sectors of the image.</summary>
    private static RockRidgeInfo? ParseRockRidge(Stream iso, ReadOnlySpan<byte> rec, int idLen)
    {
        int baseOffset = 33 + idLen;
        if ((baseOffset & 1) != 0) baseOffset++;   // identifier padded to even
        if (baseOffset >= rec.Length) return null;

        var su = rec[baseOffset..].ToArray();
        var info = RockRidge.Parse(su, (offset, length) => ReadRange(iso, offset, length));
