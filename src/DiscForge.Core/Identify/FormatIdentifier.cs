...
public static class FormatIdentifier
{
    ...
    public static FormatId Identify(Stream file)
    {
        ...
        // A 512-byte footer, for formats whose signature lives at the end (VHD, DMG).
        byte[] foot = Array.Empty<byte>();
        if (len >= 512)
        {
            foot = new byte[512];
            file.Seek(len - 512, SeekOrigin.Begin);
            file.ReadExactly(foot, 0, 512);
        }

        return IdentifyBytes(head, tail, foot, len);
    }
    ...
    public static FormatId Identify(byte[] data)
    {
        ...
        var foot = data.Length >= 512 ? data.AsSpan(data.Length - 512).ToArray() : Array.Empty<byte>();
        return IdentifyBytes(data, tail, foot, data.Length);
    }

    private static FormatId IdentifyBytes(byte[] head, byte[] tail, byte[] foot, long len)
    {
        ...
        // ECM: stripped raw-CD image ("ECM\0"). unecm/`dforge unecm` rebuilds the sectors.
        if (MagicAt(head, 0, (byte)'E', (byte)'C', (byte)'M', 0x00)) return Make("ECM", "stripped disc image", "Error Code Modeler (use unecm)");
        // Xbox 360 STFS / GOD package: CON (console), LIVE / PIRS (signed, e.g. Games on Demand).
        if (Ascii(head, 0, "CON ") || Ascii(head, 0, "LIVE") || Ascii(head, 0, "PIRS"))
            return Make("STFS", "Xbox 360 package", "CON/LIVE/PIRS (GOD; use god-info)");
        ...
        // Game Boy / Game Boy Color: the fixed Nintendo logo bitmap at 0x104.
        if (head.Length > 0x144 && MagicAt(head, 0x104, 0xCE, 0xED, 0x66, 0x66))
            return Make(head[0x143] is 0x80 or 0xC0 ? "Game Boy Color" : "Game Boy", "cartridge ROM", ".gb / .gbc");
        // Sega Master System / Game Gear: the "TMR SEGA" header tag at a fixed late offset.
        foreach (int o in new[] { 0x1FF0, 0x3FF0, 0x7FF0 })
            if (Ascii(head, o, "TMR SEGA")) return Make("Master System / Game Gear", "cartridge ROM", ".sms / .gg");
        ...
        // Apple II WOZ, Commodore 1541 GCR (G64), ZX Spectrum tape (TZX): strong ASCII magics.
        if (Ascii(head, 0, "WOZ1") || Ascii(head, 0, "WOZ2")) return Make("WOZ", "floppy image", "Apple II disk (flux)");
        if (Ascii(head, 0, "GCR-1541")) return Make("G64", "floppy image", "Commodore 1541 GCR");
        if (Ascii(head, 0, "ZXTape!")) return Make("TZX", "tape image", "ZX Spectrum tape");
        // Atari 8-bit ATR: the 0x0296 "NICKATARI" magic word (little-endian) at offset 0.
        if (head.Length > 1 && head[0] == 0x96 && head[1] == 0x02) return Make("ATR", "floppy image", "Atari 8-bit disk");
        ...
        // ---- virtual-disk / disk-image containers ----
        if (Ascii(head, 0, "conectix")) return Make("VHD", "disk image", "Microsoft Virtual Hard Disk");
        if (foot.Length == 512 && Ascii(foot, 0, "conectix")) return Make("VHD", "disk image", "Microsoft Virtual Hard Disk (fixed)");
        if (Ascii(head, 0, "vhdxfile")) return Make("VHDX", "disk image", "Microsoft Hyper-V VHDX");
        if (MagicAt(head, 0, (byte)'K', (byte)'D', (byte)'M', (byte)'V')) return Make("VMDK", "disk image", "VMware sparse disk");
        if (Ascii(head, 0, "# Disk DescriptorFile")) return Make("VMDK", "disk image", "VMware disk descriptor");
        if (Ascii(head, 0, "<<< Oracle VM VirtualBox Disk Image >>>") || MagicAt(head, 0x40, 0x7F, 0x10, 0xDA, 0xBE))
            return Make("VDI", "disk image", "Oracle VirtualBox disk");
        if (MagicAt(head, 0, (byte)'Q', (byte)'F', (byte)'I', 0xFB)) return Make("QCOW", "disk image", "QEMU copy-on-write");
        if (foot.Length == 512 && Ascii(foot, 0, "koly")) return Make("DMG", "disk image", "Apple Disk Image (UDIF)");

        // ---- non-disc filesystems (raw volume/partition dumps) ----
        if (Ascii(head, 3, "NTFS    ")) return Make("NTFS", "disk filesystem", "Windows NT File System");
        if (Ascii(head, 3, "EXFAT   ")) return Make("exFAT", "disk filesystem", "Microsoft exFAT");
        if (head.Length > 0x43A && head[0x438] == 0x53 && head[0x439] == 0xEF)
            return Make("ext2/3/4", "disk filesystem", "Linux extended filesystem");
        if (head.Length > 0x402 && head[0x400] == 0x48 && head[0x401] is 0x2B or 0x58)
            return Make("HFS+", "disk filesystem", head[0x401] == 0x58 ? "Mac OS Extended (HFSX)" : "Mac OS Extended (HFS+)");
        if (head.Length > 0x402 && head[0x400] == 0x42 && head[0x401] == 0x44)
            return Make("HFS", "disk filesystem", "classic Mac OS Standard (HFS)");
        if (MagicAt(head, 0, (byte)'h', (byte)'s', (byte)'q', (byte)'s') || MagicAt(head, 0, (byte)'s', (byte)'q', (byte)'s', (byte)'h'))
            return Make("SquashFS", "disk filesystem", "Linux SquashFS");
        ...
        if (Ascii(head, 0, "MSCF")) return Make("CAB", "archive", "Microsoft Cabinet");
        if (Ascii(head, 2, "-lh") || Ascii(head, 2, "-lz")) return Make("LZH", "archive", "LHA / LArc (Amiga/DOS)");
        if (MagicAt(head, 0, 0x04, 0x22, 0x4D, 0x18)) return Make("LZ4", "archive", "LZ4 frame");
        if (head.Length > 1 && head[0] == 0x1F && head[1] == 0x9D) return Make("compress", "archive", "Unix compress (.Z)");
        if (head.Length > 1 && head[0] == 0x60 && head[1] == 0xEA) return Make("ARJ", "archive", "ARJ archive");

        // More audio
        if (MagicAt(head, 0, 0x30, 0x26, 0xB2, 0x75)) return Make("ASF/WMA", "audio", "Advanced Systems Format (WMA/WMV)");
        if (Ascii(head, 0, "MAC ")) return Make("Monkey's Audio", "audio", "APE lossless");
        if (Ascii(head, 0, "wvpk")) return Make("WavPack", "audio", "WavPack lossless");
        if (Ascii(head, 0, "MPCK") || Ascii(head, 0, "MP+")) return Make("Musepack", "audio", "Musepack");
        if (Ascii(head, 0, "TTA1")) return Make("TTA", "audio", "True Audio lossless");
        if (Ascii(head, 0, "#!AMR")) return Make("AMR", "audio", "Adaptive Multi-Rate");

        // More images / textures
        if (Ascii(head, 0, "DDS ")) return Make("DDS", "image", "DirectDraw Surface (GPU texture)");
        if (Ascii(head, 0, "8BPS")) return Make("PSD", "image", "Adobe Photoshop");
        if (MagicAt(head, 0, 0x00, 0x00, 0x01, 0x00)) return Make("ICO", "image", "Windows icon");
        if (MagicAt(head, 0, 0x00, 0x00, 0x00, 0x0C) && Ascii(head, 4, "jP  ")) return Make("JPEG 2000", "image", "JP2");
        if (MagicAt(head, 0, 0x76, 0x2F, 0x31, 0x01)) return Make("OpenEXR", "image", "ILM OpenEXR");
        if (Ascii(head, 0, "gimp xcf")) return Make("XCF", "image", "GIMP image");
        if (head.Length > 3 && head[0] == 0x0A && head[1] is 0 or 2 or 3 or 4 or 5 && head[2] == 0x01)
            return Make("PCX", "image", "ZSoft Paintbrush");

        // Patches (DiscForge applies PPF; these other patch formats are named for reference)
        if (Ascii(head, 0, "PATCH")) return Make("IPS", "patch", "International Patching System");
        if (Ascii(head, 0, "BPS1")) return Make("BPS", "patch", "beat patch");
        if (Ascii(head, 0, "UPS1")) return Make("UPS", "patch", "Universal Patching System");
        if (head.Length > 3 && head[0] == 0xD6 && head[1] == 0xC3 && head[2] == 0xC4) return Make("xdelta", "patch", "VCDIFF / xdelta");

        // Documents
        if (Ascii(head, 0, "%PDF")) return Make("PDF", "document", "Portable Document Format");
        if (Ascii(head, 0, "{\\rtf")) return Make("RTF", "document", "Rich Text Format");
        if (MagicAt(head, 0, 0xD0, 0xCF, 0x11, 0xE0)) return Make("OLE2", "document", "Microsoft Compound File (legacy Office)");
