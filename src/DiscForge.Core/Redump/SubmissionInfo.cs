...
public sealed record SubmissionInfo
{
    ...
    /// <summary>What the disc's own SYSTEM.CNF says (serial, region, video mode) when the
    /// image is a PlayStation disc; else null. This is the part of "Common Disc Info" that
    /// IS derivable from the image — unlike the game's marketing title or the ring code.</summary>
    public PlayStation.PsDiscId? PlayStationId { get; init; }
    ...
    public string ToRedumpText()
    {
        ...
        sb.AppendLine("\tTitle: ");            // the marketing name is not in the image — submitter fills it
        sb.AppendLine(PlayStationId is not null
            ? $"\tRegion: {PlayStationId.Region}"
            : "\tRegion: ");
        sb.AppendLine("\tBarcode: ");
        sb.AppendLine();
        if (PlayStationId is not null)
        {
            sb.AppendLine("Detected from image (SYSTEM.CNF — identification only):");
            string console = PlayStationId.Console == PlayStation.PsConsole.Ps2 ? "PlayStation 2" : "PlayStation 1";
            string serial = PlayStationId.GameId.Length > 0 ? PlayStationId.GameId : "(non-standard boot file)";
            sb.AppendLine($"\tConsole: {console}   Serial: {serial}");
            sb.AppendLine($"\tBoot: {PlayStationId.BootPath}");
            if (PlayStationId.Version is not null || PlayStationId.VideoMode is not null)
                sb.AppendLine($"\tVersion: {PlayStationId.Version ?? "—"}   Video mode: {PlayStationId.VideoMode ?? "—"}");
            sb.AppendLine();
        }
...
public static class SubmissionInfoGenerator
{
    ...
    public static SubmissionInfo Generate(string path)
    {
        ...
        // The one part of "Common Disc Info" that IS in the image: a PlayStation disc's
        // own SYSTEM.CNF names its serial, region and video mode. Reading it is pure
        // identification (a plain-text file from the disc's filesystem, nothing decrypted).
        PlayStation.PsDiscId? psId = null;
        try { psId = PlayStation.SystemCnf.FromImage(path); }
        catch { /* not a PlayStation disc, or filesystem unreadable — leave the fields blank */ }
