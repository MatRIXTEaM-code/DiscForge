...
/// <see cref="ChdLzma"/>), cdfl (FLAC, <see cref="ChdFlac"/>) or cdzs (Zstandard,
/// <see cref="DiscForge.Core.Compression.ZstdDecoder"/>), auto-detected from the hunk's
/// framing — an uncompressed (NONE) hunk is taken verbatim, and a SELF
...
public static class ChdExtractor
{
    ...
    public static CdExtraction ExtractCd(byte[] chd, byte[][] parentChain)
    {
        ...
        foreach (var comp in info.Compressors)
            if (comp is not ("cdzl" or "cdlz" or "cdfl" or "cdzs" or "none"))
                throw new ChdFormatException(
                    $"This CHD uses the '{comp}' codec, which isn't supported. Only the CD codecs " +
                    "cdzl (zlib), cdlz (LZMA), cdfl (FLAC) and cdzs (Zstandard) are decoded. Convert it to " +
                    "bin/cue with chdman instead.");
        ...
        byte[] raw = DecodeCdLogical(chd, info, parentChain, out bool verified);
        var (bin, cue, tracks) = BuildBinCue(raw, info);
        // Verified is true only when the CHD carried a SHA-1 that the decoded data actually matched;
        // an uncompressed CHD stores none, so the extraction is correct-by-construction but unverified.
        return new CdExtraction { Bin = bin, Cue = cue, Tracks = tracks, Verified = verified };
    }
    ...
    private static byte[] DecodeCdLogical(byte[] chd, ChdInfo info, byte[][] parentChain, out bool verified)
    {
            ...
            switch (entry.Type)
            {
                ...
                default:  // Parent
                {
                    ...
                    if (parentRaw is null)
                    {
                        ...
                        parentRaw = DecodeCdLogical(parentBytes, parentInfo, parentChain[1..], out _);   // resolve up the chain
                    }
        ...
        verified = hasStoredSha1;   // only a matched stored SHA-1 counts as verified
        return raw;
    }
    ...
    /// <summary>zstd frame magic (little-endian 0xFD2FB528) at <paramref name="pos"/>.</summary>
    private static bool IsZstd(byte[] d, int pos) =>
        pos + 4 <= d.Length && d[pos] == 0x28 && d[pos + 1] == 0xB5 && d[pos + 2] == 0x2F && d[pos + 3] == 0xFD;
        ...
        out byte[]? sect, out byte[]? sub, out byte eccFlags, out int nextPos)
    {
        ...
        try
        {
            ...
            {
                // cdzl / cdlz / cdzs: [ecc flags][2-byte base length][base stream].
                eccFlags = chd[pos];
                int sourceLen = (chd[pos + 1] << 8) | chd[pos + 2];
                int basePos = pos + 3;
                if (basePos + 4 > chd.Length) return false;
                sect = IsZstd(chd, basePos)                       // zstd frame magic 28 B5 2F FD
                    ? Compression.ZstdDecoder.Decompress(chd.AsSpan(basePos, sourceLen))
                    : chd[basePos] == 0x00                        // LZMA range-coder init byte
                        ? ChdLzma.Decode(chd, basePos, framesPerHunk * Sector)
                        : new ChdInflate(chd, basePos).Run();
                subStart = basePos + sourceLen;
            }

            if (subStart + 4 > chd.Length) return false;
            // The subcode stream carries its own codec: zstd (cdzs) or zlib (cdzl/cdlz/cdfl),
            // detected by magic so the framing needn't be assumed from the sector codec.
            if (IsZstd(chd, subStart))
            {
                sub = Compression.ZstdDecoder.DecompressFrame(chd.AsSpan(subStart), out int used);
                nextPos = subStart + used;
            }
            else
            {
