...
///
/// Mode 1 (<see cref="CorrectMode1"/>) and Mode 2 Form 1 (<see cref="CorrectMode2Form1"/>,
/// the PlayStation / CD-XA data form) are both handled — Form 1 differs only in
/// that its parity is computed with the header zeroed, so that header is blanked
/// and locked for the decode. Mode 2 Form 2 carries no ECC at all and cannot be
/// repaired this way — for those, C2 voting across re-reads is all there is.
/// </summary>
public static class EccCorrector
{
        ...
        bool correctUnflaggedErrors = true)
    {
        ...
        int corrected = RepairSync(sector, erased);
        int passes = RunProductPasses(sector, erased, locked: null, maxPasses,
                                      correctUnflaggedErrors, ref corrected);

        var (edcOk, _) = EdcEcc.VerifyMode1(sector);
        return BuildResult(erased, edcOk, corrected, passes);
    }

    /// <summary>
    /// Repair a raw <b>Mode 2 Form 1</b> sector in place — the form a PlayStation
    /// (or any CD-XA) data disc stores its filesystem in. Identical erasure-guided
    /// RSPC decoding to <see cref="CorrectMode1"/>, with the one CD-XA difference:
    /// the 4-byte header is excluded from the parity (computed as if zero), so it
    /// is blanked and locked for the decode and the real address is restored
    /// afterwards. EDC is checked over the sub-header + user data (16..2071).
    /// </summary>
    public static EccCorrectionResult CorrectMode2Form1(
        Span<byte> sector,
        IReadOnlyList<int> erasures,
        int maxPasses = 8,
        bool correctUnflaggedErrors = true)
    {
        ...
        // CD-XA leaves the header out of the ECC, so the parity was laid down with
        // it zeroed. Match that: save the address, blank it, and lock those four
        // bytes so no codeword tries to "correct" them; restore it on the way out.
        Span<byte> savedHeader = stackalloc byte[4];
        sector.Slice(12, 4).CopyTo(savedHeader);
        sector.Slice(12, 4).Clear();

        var locked = new bool[SectorBytes];
        for (int i = 12; i < 16; i++) { locked[i] = true; erased[i] = false; }

        int passes = RunProductPasses(sector, erased, locked, maxPasses,
                                      correctUnflaggedErrors, ref corrected);

        savedHeader.CopyTo(sector.Slice(12, 4));   // put the real address back

        var (edcOk, _) = EdcEcc.VerifyMode2Form1(sector);
        return BuildResult(erased, edcOk, corrected, passes);
    }

    /// <summary>Sync bytes carry no parity but are a fixed constant — a flagged
    /// sync byte is repaired by writing what it must be. Returns how many changed.</summary>
    private static int RepairSync(Span<byte> sector, bool[] erased)
    {
        int corrected = 0;
    ...
    /// <summary>Alternate P and Q passes until nothing more changes (or the guard
    /// limit is hit), accumulating corrections. Returns the pass count used.</summary>
    private static int RunProductPasses(Span<byte> sector, bool[] erased, bool[]? locked,
                                        int maxPasses, bool correctUnflagged, ref int corrected)
    {
        int passes = 0;
    ...
    /// <summary>Assemble the result record and its human-readable detail from the
    /// post-decode state — shared by both sector forms.</summary>
    private static EccCorrectionResult BuildResult(bool[] erased, bool edcOk, int corrected, int passes)
    {
    ...
    private static int CorrectCodeword(Span<byte> sector, ReadOnlySpan<int> offsets,
                                       bool[] erased, bool correctUnflagged, bool[]? locked = null)
    {
        ...
        switch (count)
        {
            case 2:
            {
                ...
                if (locked != null && (locked[offsets[e0]] || locked[offsets[e1]])) return 0;
            ...
            case 1:
            {
                ...
                if (locked != null && locked[offsets[e0]]) return 0;
            ...
            case 0:
            {
                ...
                if (locked != null && locked[offsets[pos]]) return 0;  // never touch a locked byte
