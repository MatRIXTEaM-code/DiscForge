...
public sealed record DatMatch
{
    ...
    /// <summary>How strong the evidence behind a verification is: "sha1" (collision-resistant) or
    /// "crc32" (32-bit — very probably right, but not cryptographic proof; a deliberate collision at
    /// fixed size is feasible). Null when not verified. Anything presenting a "confirmed good dump"
    /// claim should prefer sha1 strength or say which it had.</summary>
...
public sealed class DatFile
{
    ...
    private readonly Dictionary<string, List<DatRom>> _byMd5 = new(StringComparer.OrdinalIgnoreCase);
    ...
    private DatFile(string? name, List<DatRom> roms)
    {
        ...
        foreach (var r in roms)
        {
            ...
            if (r.Md5 is not null) Index(_byMd5, r.Md5, r);
        }
    }
    ...
    public IReadOnlyList<DatRom> ByMd5(string md5) =>
        _byMd5.TryGetValue(md5.Trim(), out var l) ? l : Array.Empty<DatRom>();
    ...
    public DatMatch Verify(long size, string? crc, string? sha1 = null, string? md5 = null)
    {
        ...
        if (sha1 is not null)
        {
            foreach (var r in BySha1(sha1))
                if (r.Size == 0 || r.Size == size)
                    return new DatMatch { Verified = true, Rom = r, Strength = "sha1",
                                          Reason = "SHA-1 and size match a catalogued dump." };
        }

        // MD5 next: weaker than SHA-1 but far stronger than CRC-32, and some DATs carry only MD5.
        if (md5 is not null)
        {
            foreach (var r in ByMd5(md5))
                if ((r.Size == 0 || r.Size == size) &&
                    (sha1 is null || r.Sha1 is null || string.Equals(r.Sha1, sha1, StringComparison.OrdinalIgnoreCase)))
                    return new DatMatch { Verified = true, Rom = r, Strength = "md5",
                                          Reason = "MD5 and size match a catalogued dump." };
        }
        ...
        if (crc is not null)
        {
            ...
            foreach (var r in crcHits)
            {
                ...
                if (sizeOk && sha1Ok)
                    return new DatMatch { Verified = true, Rom = r, Strength = "crc32",
                                          Reason = "CRC-32 and size match a catalogued dump (32-bit evidence — " +
                                                   "supply a SHA-1 for collision-resistant confirmation)." };
            }
            if (crcHits.Count > 0)
                return new DatMatch
                {
