...
public static class XdvdfsReader
{
    ...
    public static XdvdfsVolume Read(Stream image, long? baseSector = null)
    {
        ...
        long baseLba = baseSector ?? FindBase(image)
            ?? throw new XdvdfsFormatException(
                "No XDVDFS volume descriptor found — the \"MICROSOFT*XBOX*MEDIA\" signature is not " +
                "at sector 32 of any known game-partition base. This does not look like an Xbox " +
                "game image (or it is a raw dump with an offset not yet supported).");
    ...
    private static long? FindBase(Stream image)
    {
        ...
        {
            long at = (b + VolumeDescriptorSector) * SectorSize;
            if (at + SectorSize > image.Length) continue;
            var vd = ReadSector(image, b + VolumeDescriptorSector);
            // Both the leading signature and the trailer must match — the trailer
            // guards against a stray copy of the string in file data.
            if (StartsWith(vd, 0, Magic) && StartsWith(vd, 0x7EC, Magic))
                return b;
        }
        return null;
    }
