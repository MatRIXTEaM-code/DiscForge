        ...
        _tiles = new Tile[]
        {
           ...
           new("recovery","Recovery",      C(0x50,0xB0,0x90), C(0x1E,0x6E,0x55), "🩹",
            ...
            new("mergecert","Merge + Certify", C(0x50,0xB0,0x50), C(0x1E,0x6E,0x1E), "🔏",
                () => new Views.MergeCertView(), "Merge rips into a signed, auditable provenance certificate"),
            new("dumpcert","Dump Certificate", C(0x50,0x90,0x50), C(0x1E,0x50,0x1E), "📜",
                () => new Views.DumpCertView(), "Certify or verify a single dump's SHA-256 + Merkle root"),
            new("prove",   "Prove",            C(0x90,0x50,0x50), C(0x50,0x1E,0x1E), "🔥",
                () => new Views.ProveView(), "Burn + read back + verify byte-for-byte, one verdict (BURNS a disc)"),
            new("pressingdna","Pressing DNA",  C(0x50,0x70,0x90), C(0x1E,0x3C,0x50), "🧬",
                () => new Views.PressingDnaView(), "Fingerprint a pressing offline; compare two cues for SAME/DIFFERENT pressing"),
            new("drivedossier","Drive Dossier", C(0x80,0x70,0x50), C(0x48,0x3C,0x1E), "📋",
                () => new Views.DriveDossierView(), "Per-drive memory: observed quirks accumulate into warnings"),
            new("discactuary","Disc Actuary",   C(0xB0,0x70,0x50), C(0x70,0x3C,0x1E), "⏳",
                () => new Views.DiscActuaryView(), "How long a disc has left, from its scan history; rank the shelf by urgency"),
            new("discmri",  "Disc MRI",         C(0x90,0x40,0x70), C(0x50,0x1E,0x40), "🩻",
                () => new Views.DiscMriView(), "Polar damage map on the physical disc — scratches, rings, rot, voids"),
            new("secureripplan","Secure-Rip Plan", C(0x70,0x60,0xB0), C(0x38,0x28,0x70), "🗺",
                () => new Views.SecureRipPlanView(), "Grade rip evidence (VERIFIED/CONSISTENT/SUSPECT/FAILED) and plan re-reads"),
            new("dumpledger","Dump Ledger",   C(0x60,0x80,0x50), C(0x2C,0x48,0x1E), "🧾",
                () => new Views.DumpCertificateLedgerView(), "A public, hash-chained log of independently signed dump claims — verify, submit, check consensus"),
            new("mediamortality","Media Decay", C(0x80,0x60,0x60), C(0x48,0x28,0x28), "📉",
                () => new Views.MediaMortalityView(), "Federated model of how fast a cohort of discs decays — observe, merge, estimate (privacy-floored)"),
...
new("pack",    "Pack Discs",    C(0x70,0xA0,0x80), C(0x30,0x60,0x48), "📦",
            ...
            new("floppy",  "Floppy",        C(0x8B,0x6C,0xC0), C(0x4E,0x28,0x76), "💾",
                () => new Views.FloppyView(), "Image a floppy natively, or launch KryoFlux/Greaseweazle for flux capture"),
            new("psp",     "PSP",           C(0x6C,0x8B,0xC0), C(0x28,0x4E,0x76), "🎮",
                () => new Views.PspView(), "Launch UMDGen to edit a PSP ISO"),
            new("cart",    "Cartridges",    C(0xC0,0x9C,0x6C), C(0x76,0x54,0x28), "🕹",
                () => new Views.CartridgeView(), "Launch GBxCart RW/FlashGBX or Cart Reader to dump a cartridge"),
            new("formatmedia","Format Media", C(0x90,0x90,0x60), C(0x50,0x50,0x28), "💳",
                () => new Views.FormatMediaView(), "Launch a card formatter (e.g. SD Card Formatter) to prep an SD/SDHC/SDXC card"),
            new("rawcopy", "Raw Copy",       C(0x70,0x70,0x90), C(0x38,0x38,0x50), "🗄",
                () => new Views.RawCopyView(), "Launch a sector-level drive/image cloning tool (e.g. HDD Raw Copy Tool)"),
            new("parity",  "Protect Image", C(0x3C,0x9C,0xC0), C(0x0F,0x58,0x7A), "🔰",
                () => new Views.ProtectImageView(), "Parity file that can repair an image later"),
            new("bitrot",  "Bit-Rot Watch", C(0xB0,0x5C,0x9C), C(0x6A,0x1E,0x5C), "👁",
                () => new Views.BitRotWatchView(), "Spot files in a collection that silently changed"),
            new("compare", "Compare Images", C(0x7C,0x8C,0xC0), C(0x3A,0x46,0x80), "⚖",
                () => new Views.CompareImagesView(), "What's different between two images"),
            new("checksums","Checksum Files", C(0x6C,0xB0,0x5C), C(0x2E,0x6A,0x22), "🔢",
                () => new Views.ChecksumFilesView(), "Make / check .sfv .md5 .sha1, check PAR2"),
            new("find",    "Find in Image", C(0xC0,0xA0,0x4C), C(0x7A,0x5E,0x12), "🔎",
                () => new Views.FindInImageView(), "Search an image for text or bytes"),
            new("health",  "Sector Health", C(0xC0,0x5C,0x6C), C(0x7A,0x1E,0x2E), "🩺",
                () => new Views.SectorHealthView(), "Map and repair a raw CD image's sectors"),
            new("bestof",  "Best of Dumps", C(0x5C,0xB0,0xA8), C(0x1E,0x6A,0x62), "🧪",
                () => new Views.BestOfDumpsView(), "Build one verified image from several dumps"),
