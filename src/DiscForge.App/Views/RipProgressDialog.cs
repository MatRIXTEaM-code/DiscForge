...
{
    // Status can be a long line ("burn: Writing raw image (DAO) — this cannot be cancelled safely"):
    // bound its width, ellipsize what overflows, and carry the full text in a tooltip.
    private readonly Label _statusVal = new() { AutoSize = false, AutoEllipsis = true, Location = new Point(140, 20), Size = new Size(404, 20), Font = Theme.Ui };
    ...
    {
        Location = new Point(16, 146), Size = new Size(528, 18), Minimum = 0, Maximum = 100,
    };
    private readonly Button _cancel = new()
    {
        Text = "Cancel", Location = new Point(464, 172), Width = 80, Height = 26, FlatStyle = FlatStyle.System,
    };
    ...
    public void SetStats(string status, string rate, string sectors, string timeRemaining) => Ui(() =>
    {
        ...
        _tip.SetToolTip(_statusVal, status);   // full text on hover when the line is ellipsized
        _rateVal.Text = rate;
