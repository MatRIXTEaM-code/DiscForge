...
internal sealed class BurnView : UserControl
{
                                         ...
                                         IProgress<BurnProgress> progress)
    {
        ...
        switch (step.Kind)
        {
            case BurnStepKind.Verify:
                if (_sourceIsCue)
                    await VerifyRawDiscAsync(drive, progress);
                else
    ...
    /// </summary>
    private async Task VerifyRawDiscAsync(DriveCapabilities drive, IProgress<BurnProgress> progress)
    {
        _ = progress;
        var letter = DriveLetterOf(drive)
            ?? throw new InvalidOperationException($"No drive letter in '{drive.DevicePath}'.");
    ...
    /// </summary>
    private async Task TestRawDiscAsync(DriveCapabilities drive)
    {
        var letter = DriveLetterOf(drive)
            ?? throw new InvalidOperationException($"No drive letter in '{drive.DevicePath}'.");
