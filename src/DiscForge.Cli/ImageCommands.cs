    ...
    public static int EccRepair(string[] args)
    {
        if (args.Length < 2)
            return Fail("usage: dforge ecc-repair <image.bin> [--out <repaired.bin>] [--dry-run]\n" +
                        "  Checks every Mode 1 / Mode 2 Form 1 sector's EDC and rebuilds damaged ones\n" +
                        "  from the Reed-Solomon parity the sector already carries. Raw 2352-byte images only.");
        if (!File.Exists(args[1])) return Fail($"File not found: {args[1]}");
        ...
        try
        {
            for (long s = 0; s < sectors; s++)
            {
                ...
                if (!mode1 && !mode2Form1)
                {
                    // Audio, Mode 2 Form 2, or an empty sector: no correctable
                    // parity to work with, so it passes through untouched.
                    skipped++;
                ...
                checkedCount++;
                var (edcOk, _) = mode1 ? EdcEcc.VerifyMode1(sector)
                                       : EdcEcc.VerifyMode2Form1(sector);
                if (edcOk)
                {
                    alreadyGood++;
                    output?.Write(sector, 0, 2352);
                ...
                var result = mode1 ? EccCorrector.CorrectMode1(sector, Array.Empty<int>())
                                   : EccCorrector.CorrectMode2Form1(sector, Array.Empty<int>());
                if (result.Success)
                {
                    repaired++;
                }
        ...
        Console.WriteLine("\r        ");
        Console.WriteLine($"  ECC sectors checked    : {checkedCount:N0}");
        ...
        Console.WriteLine($"  skipped (no ECC)       : {skipped:N0}");
        Console.WriteLine();
