...
public static class C2SectorReader
{
                                            ...
                                            C2ReadOptions? options = null)
    {
        ...
        //
        // Mode 1 only: byte 15 of the header says which, and Mode 2 Form 2
        // carries no parity to work with.
        if (opts.UseEccCorrection && uncertain.Count > 0 && successes > 0 && IsMode1(sector))
        {
            var repaired = (byte[])sector.Clone();
            var result = EccCorrector.CorrectMode1(repaired, uncertain);
    ...
    /// <summary>True when the sector's own header says Mode 1 — the only mode
    /// whose parity this can use.</summary>
