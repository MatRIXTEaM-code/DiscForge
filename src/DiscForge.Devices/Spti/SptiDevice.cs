        ...
        unsafe
        {
            fixed (byte* dataPtr = dataBuffer)
            {
                ...
                try
                {
                    ...
                    // Capture the OS error immediately, before any other call clobbers it.
                    int win32 = ok ? 0 : Marshal.GetLastWin32Error();

                    var outWrapper = Marshal.PtrToStructure<SPTD_WITH_SENSE>(native);
                    if (!ok)
                        return new SptiResult(false, outWrapper.Spt.ScsiStatus, outWrapper.Sense, win32);
...
/// </summary>
public readonly record struct SptiResult(bool Success, byte ScsiStatus, byte[] SenseData, int Win32Error = 0)
{
    /// <summary>The failure came from the OS/driver (DeviceIoControl returned false)
    /// rather than a SCSI CHECK CONDITION — no sense data, just a Win32 error. Often a
    /// transient link or spin-up glitch (common over IDE-to-USB bridges) that a retry clears.</summary>
    ...
    public string Describe()
    {
        ...
        // A driver-level failure carries no SCSI sense — report the OS error and its
        // usual, non-alarming cause instead of a misleading "No sense (status 0x00)".
